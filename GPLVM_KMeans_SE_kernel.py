...
params.add_argument('-num_iterations', type=int, default=2, help='iteration time ')
...
output_scale_layer1 = torch.nn.Parameter(torch.tensor([output_scale_model].to(device), dtype=torch.float64).to(device), requires_grad=True)
length_scale_layer1 = torch.nn.Parameter(torch.tensor([length_scale_model].to(device), dtype=torch.float64).to(device), requires_grad=True)
noise_variance = torch.nn.Parameter(torch.tensor([noise_model].to(device), dtype=torch.float64).to(device), requires_grad=True)
...
K_train_train += torch.eye(X_test.size(0)) * noise_variance_optimize ** 2
...
file_name = f'GPLVM(data_{N}_Q_{Q}_D_{D})_KMeans_LS{length_scale_generate}_OP{output_scale_generate}_IP{M}_Iter{num_iterations}_LearningR{learning_rate}.pt'
torch.save(loss_set, file_name)
...
plt.title(f" GPLVM(data_{N}_Q_{Q}_D_{D})_KMeans_LS{length_scale_generate}_OP{output_scale_generate}_IP{M}_Iter{num_iterations}_LearningR{learning_rate} ")
plt.xlabel('iteration time k')
plt.ylabel('loss')
# plt.savefig(f'GPLVM(data_{N}_Q_{Q}_D_{D})_KMeans_LS{length_scale_generate}_OP{output_scale_generate}_IP{M}_Iter{num_iterations}_LearningR{learning_rate}.png')
plt.show()